...
@Composable
fun SportsApp(
    windowSize: WindowWidthSizeClass,          // AGREGA
    modifier: Modifier = Modifier
) {
    val viewModel: SportsViewModel = viewModel()
    val uiState by viewModel.uiState.collectAsState()

    // AGREGA: decidir el tipo de contenido según el ancho
    val contentType = when (windowSize) {
        WindowWidthSizeClass.Expanded -> SportsContentType.ListAndDetail
        else -> SportsContentType.ListOnly
    }
    val activity = LocalContext.current as Activity   // AGREGA

    Scaffold(
        topBar = {
            SportsAppBar(
                // CAMBIA: en expandida siempre se comporta como "página de lista"
                // → título "Deportes" y sin botón Atrás
        ...
        if (contentType == SportsContentType.ListAndDetail) {   // AGREGA toda esta rama
            ...
            // ↓ esto es EXACTAMENTE lo que ya tenías (comportamiento anterior)
